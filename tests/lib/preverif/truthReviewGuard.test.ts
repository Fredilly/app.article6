...
function makeHistoricalRepo(withHistorical = false): string {
  ...
  const historicalSource = path.join(repoRoot, "tests/fixtures/preverif/marcondes-vm0007-v18-evidence-map/gold.rc2-rc3.json");
  const baseGold = fs.readFileSync(historicalSource);
  const baseGoldSha = crypto.createHash("sha256").update(baseGold).digest("hex");
  if (baseGoldSha !== "b53fc19a8316f88896b7f9564a8e2d2d0dd8b08c9e05868a7b427140f47e1127") throw new Error(`Unexpected immutable historical truth SHA: ${baseGoldSha}`);
...
const scenarios: Scenario[] = [
  ...
  { name: "passes guard-only infrastructure maintenance", expectPass: true, mutate: (d) => { fs.mkdirSync(path.join(d, "scripts/preverif"), { recursive: true }); fs.writeFileSync(path.join(d, "scripts/preverif/check-truth-review-guard.mjs"), "changed\n"); } },
  { name: "passes generator-only infrastructure maintenance", expectPass: true, mutate: (d) => { fs.mkdirSync(path.join(d, "scripts/preverif"), { recursive: true }); fs.writeFileSync(path.join(d, "scripts/preverif/generate-vm0007-rc2-baseline.ts"), "changed\n"); } },
  { name: "blocks truth plus guard outside migration", expectPass: false, message: "actual historical truth file creation migration", mutate: (d) => { fs.mkdirSync(path.join(d, "scripts/preverif"), { recursive: true }); fs.writeFileSync(path.join(d, "scripts/preverif/check-truth-review-guard.mjs"), "changed\n"); const g = readJson(d, "tests/fixtures/preverif/demo/gold.json"); g.rows.push(validSecond); syncGold(d, g.rows); } },
  { name: "blocks truth plus protected generator outside migration", expectPass: false, message: "actual historical truth file creation migration", mutate: (d) => { fs.mkdirSync(path.join(d, "scripts/preverif"), { recursive: true }); fs.writeFileSync(path.join(d, "scripts/preverif/generate-vm0007-rc2-baseline.ts"), "changed\n"); const g = readJson(d, "tests/fixtures/preverif/demo/gold.json"); g.rows.push(validSecond); syncGold(d, g.rows); } },
