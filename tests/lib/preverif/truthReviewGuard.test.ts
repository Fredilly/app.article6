...
const scenarios: Scenario[] = [
  ...
  { name: "blocks guard-only self-modification outside migration", expectPass: false, message: "requires the one-time historical truth migration", mutate: (d) => { fs.mkdirSync(path.join(d, "scripts/preverif"), { recursive: true }); fs.writeFileSync(path.join(d, "scripts/preverif/check-truth-review-guard.mjs"), "changed\n"); } },
  { name: "blocks generator-only self-modification outside migration", expectPass: false, message: "requires the one-time historical truth migration", mutate: (d) => { fs.mkdirSync(path.join(d, "scripts/preverif"), { recursive: true }); fs.writeFileSync(path.join(d, "scripts/preverif/generate-vm0007-rc2-baseline.ts"), "changed\n"); } },
