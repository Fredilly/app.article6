...
type Scenario = { name: string; expectPass: boolean; mutate?: (dir: string) => void; message?: string; legacyBaseline?: boolean; noAudit?: boolean };
...
function makeRepo(legacyBaseline = false, noAudit = false): string {
...
function runScenario(scenario: Scenario): { status: number; output: string } {
    const dir = makeRepo(scenario.legacyBaseline, scenario.noAudit);
  try {
    scenario.mutate?.(dir); git(dir, ["add", "."]); git(dir, ["commit", "-qm", "head"]);
    try { execFileSync("node", [guard, "--base-ref", "HEAD~1"], { cwd: dir, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }); return { status: 0, output: "" }; } catch (error: any) { return { status: error.status ?? 1, output: `${error.stdout ?? ""}${error.stderr ?? ""}` }; }
