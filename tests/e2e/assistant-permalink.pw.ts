...
test("Assistant evidence permalinks restore focus on reload", async ({ page }) => {
  ...
  await expect(page.locator(`#${sectionId}.assistant-focus-highlight`)).toBeVisible({ timeout: 5_000 });

  await page.reload();

  await expect(page).toHaveURL(new RegExp(`\\btab=sections\\b`));
  await expect(page).toHaveURL(new RegExp(`\\bfocus=${sectionId}\\b`));
  await expect(page.locator(`#${sectionId}.assistant-focus-highlight`)).toBeVisible({ timeout: 15_000 });
});
