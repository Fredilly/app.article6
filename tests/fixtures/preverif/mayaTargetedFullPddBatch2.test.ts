...
describe("RC5-2 Maya targeted full-PDD batch 2", () => {
  ...
  it("keeps the frozen packet inventory pinned while integrated truth is 43 reviewed / 15 provisional", () => {
    ...
    assert.equal(before.filter((r: any) => r.reviewStatus === "REVIEWED").length, 43);
    assert.equal(before.filter((r: any) => r.reviewStatus === "PROVISIONAL").length, 15);
