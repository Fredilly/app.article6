...
const preChangeFileSha256: Record<string, string> = {
  ...
  "docs/roadmaps/interactive-evidence-review-mvp/rc/rc5/rc5-2-maya-batch-5-adjudication/reviewed-truth.json": "1cce490339d408f314b42a3238f4cefe3b416ab17cdaf6c6673bb35f196afc5f",
};
...
describe("RC5-2 Maya Batch 6 final reviewed truth", () => {
  ...
  it("covers exactly 58 unique rules and blocks RC5 completion while current provisional rows remain", () => {
    ...
    assert.equal(decisions.filter((decision) => decision.reviewStatus === "REVIEWED").length, 50);
    assert.equal(decisions.filter((decision) => decision.reviewStatus === "PROVISIONAL").length, 8);
