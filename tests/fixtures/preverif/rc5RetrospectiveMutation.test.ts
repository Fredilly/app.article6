...
describe("RC5 retrospective audit mutation coverage", () => {
  ...
  it("rejects exact-evidence mutations and unrelated packet sources against current truth", () => {
    ...
    expect(allRows.filter((decision: any) => decision.reviewStatus === "REVIEWED")).toHaveLength(51);
    expect(allRows.filter((decision: any) => decision.reviewStatus === "PROVISIONAL")).toHaveLength(7);
  });
