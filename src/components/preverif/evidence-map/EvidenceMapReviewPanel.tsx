...
export default function EvidenceMapReviewPanel({ row, finalized, onClose, onEdit, onApprove, onReopen }: Props) {
  ...
  const [editingField, setEditingField] = useState<"gap" | "clientAction" | null>(null);
  ...
  const beginFieldEdit = (field: "gap" | "clientAction") => { setEditingField(field); setFieldDraft(form[field]); setError(null); };
  const cancelFieldEdit = () => { setEditingField(null); setFieldDraft(""); setError(null); };
  const saveField = () => {
    if (!form.note.trim()) { setError("Add a review note so this change is preserved in review history."); return; }
    const result = onEdit({ [editingField!]: fieldDraft } as Vm0007EvidenceMapEdit, form.note.trim());
    if (result) setError(result); else cancelFieldEdit();
  };
  const requireNote = () => {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/30 backdrop-blur-[1px]" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={panelRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} className="evidence-map-review-panel h-full w-full max-w-2xl overflow-y-auto bg-white shadow-2xl outline-none">
      ...
      <div className="space-y-8 px-5 py-6 sm:px-8">
        ...
        <section aria-labelledby={`${titleId}-assessment`}><h3 id={`${titleId}-assessment`} className="text-xs font-semibold uppercase tracking-[.12em] text-slate-500">Reviewer assessment</h3>{!currentAssessment ? <p role="status" className="mt-3 border-l-2 border-amber-500 bg-amber-50 px-3 py-2 text-sm text-amber-900">Assessment needs review after this change.</p> : null}<div className="mt-4 grid gap-4 sm:grid-cols-2">{select("Applicability", "applicability", ["APPLICABLE", "NOT_APPLICABLE", "NOT_EVALUATED"])}<label className="grid gap-1.5 text-sm text-slate-700"><span className="font-medium">Applicability basis</span><input value={form.applicabilityBasis} onChange={(event) => set("applicabilityBasis", event.target.value)} disabled={finalized} className="min-h-10 rounded-lg border border-slate-300 px-3 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></label>{select("Requirement support", "requirementSupport", ["SUPPORTED", "NOT_SUPPORTED", "NOT_EVALUATED"])}{select("Search coverage", "searchCoverage", ["ADEQUATE", "INADEQUATE", "NOT_REQUIRED", "NOT_EVALUATED"])}{select("Provenance", "provenance", ["COMPLETE", "INCOMPLETE", "NOT_EVALUATED"])}{select("Version identity", "versionIdentity", ["MATCHED", "NOT_REQUIRED", "MISMATCHED", "UNRESOLVED"])}{select("Contradiction", "contradiction", ["NONE", "BLOCKING", "NOT_EVALUATED"])}{select("Draft finding", "draftFinding", ["NIR_CANDIDATE", "NCR_CANDIDATE", "OFI_CANDIDATE"])}<label className="grid gap-1.5 text-sm text-slate-700 sm:col-span-2"><span className="font-medium">Assessment reason</span><textarea value={form.assessmentReason} onChange={(event) => set("assessmentReason", event.target.value)} disabled={finalized} rows={3} className="rounded-lg border border-slate-300 p-3 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></label>{form.draftFinding ? <><label className="grid gap-1.5 text-sm text-slate-700"><span className="font-medium">Finding basis</span><textarea value={form.findingBasis} onChange={(event) => set("findingBasis", event.target.value)} disabled={finalized} rows={3} className="rounded-lg border border-slate-300 p-3 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></label><label className="grid gap-1.5 text-sm text-slate-700"><span className="font-medium">Reviewer assessment</span><textarea value={form.reviewerAssessment} onChange={(event) => set("reviewerAssessment", event.target.value)} disabled={finalized} rows={3} className="rounded-lg border border-slate-300 p-3 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></label></> : null}</div></section>
        <section aria-label="Gap and client action" className="grid gap-4 border-t border-slate-200 pt-5"><h3 className="text-xs font-semibold uppercase tracking-[.12em] text-slate-500">Gap and client action</h3>{(["gap", "clientAction"] as const).map((field) => <div key={field} className="grid gap-2 text-sm"><div className="flex items-center justify-between gap-3"><span className="font-medium text-slate-800">{field === "gap" ? "Gap" : "Client action"}</span>{!finalized && !editingField ? <button type="button" onClick={() => beginFieldEdit(field)} className="text-sm font-medium text-blue-700 underline underline-offset-2">Edit</button> : null}</div>{editingField === field ? <><textarea autoFocus value={fieldDraft} onChange={(event) => setFieldDraft(event.target.value)} rows={3} aria-label={`Edit ${field === "gap" ? "gap" : "client action"}`} className="rounded-md border border-slate-300 p-2 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /><div className="flex gap-2"><button type="button" onClick={saveField} disabled={finalized} className="rounded-md bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Save</button><button type="button" onClick={cancelFieldEdit} className="rounded-md px-3 py-2 text-sm text-slate-700">Cancel</button></div></> : <p className="whitespace-pre-wrap text-slate-700">{form[field] || (field === "clientAction" ? "None specified" : "No gap recorded.")}</p>}</div>)}</section>
