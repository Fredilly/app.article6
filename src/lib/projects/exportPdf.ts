...
export function buildProjectExportPdf(project: Project, coverage: ProjectCoverage): Buffer {
  ...
  function flush(): void {
    ...
    const hdr = [
      ...
      LN(HEADER_Y - 6),
      ...TXT(L, HEADER_Y, 'FB', 9, 'app.article6', '0.4 0.4 0.4 rg'),
      ...TXT(L + 84, HEADER_Y, 'F1', 8, project.name, '0.25 0.25 0.25 rg'),
      ...TXT(R - 26, HEADER_Y, 'F1', 8, pageNumber, '0.4 0.4 0.4 rg'),
      LN(FOOTER_Y + 10),
      ...TXT(L, FOOTER_Y, 'F1', 7, `Generated ${now}`, '0.6 0.6 0.6 rg'),
      ...TXT(R - 170, FOOTER_Y, 'F1', 7, 'Generated by app.article6', '0.6 0.6 0.6 rg'),
    ];
  ...
  ln.push(
    ...TXT(L, y + 10, 'FB', 16, project.name, '0.1 0.1 0.1 rg'),
    ...TXT(L, y - 10, 'F1', 9, `${project.methodCode} @ ${project.methodVersion}`, '0.35 0.35 0.35 rg'),
    ...TXT(L, y - 26, 'F1', 8, `Status: ${project.status}`, '0.45 0.45 0.45 rg'),
  );
  if (project.aoiLabel) {
    ln.push(...TXT(L + 180, y - 26, 'F1', 8, `AOI: ${project.aoiLabel}`, '0.45 0.45 0.45 rg'));
  }
  y -= 56;
  ...
  for (const [sid, reviews] of Object.entries(grouped)) {
    ...
    for (const [index, r] of reviews.entries()) {
      need(16);
      const t = r.ruleTitle.length > 60 ? r.ruleTitle.slice(0, 57) + '...' : r.ruleTitle;
      ...
      ln.push(
        ...
        ...TXT(L + 402, y, 'FB', 7, r.status === 'not-started' ? 'PENDING' : r.status.toUpperCase(), statusColor),
      );
      y -= 16;
    }
    y -= 8;
  ...
  if (gaps.length > 0) {
    ...
    for (const r of gaps) {
      ...
      ln.push(
        ...TXT(L, y, 'F1', 8, r.ruleId, '0.4 0.4 0.4 rg'),
        ...TXT(L + 110, y, 'F1', 8, t, '0.15 0.15 0.15 rg'),
        ...TXT(L + 390, y, 'F1', 8, r.status, '0.5 0.5 0.5 rg'),
      );
      y -= 16;
    }
    y -= 8;
