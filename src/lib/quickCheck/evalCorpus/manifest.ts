...
const questionExpectationSchema = z.object({
  ...
  expectedVisibleAnswerContains: z.array(z.string().min(1)).optional(),
  expectedEvidenceContains: z.array(z.string().min(1)).optional(),
  forbiddenAnswerContains: z.array(z.string().min(1)).optional(),
  forbiddenStatus: z.array(z.enum(["answered", "unclear", "no_evidence"])).optional(),
  maxVisibleAnswerLength: z.number().int().positive().optional(),
