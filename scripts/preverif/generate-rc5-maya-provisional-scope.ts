...
export function buildScopeManifest() {
  ...
  if (provisional.length !== 19) throw new Error(`Expected 19 provisional rules, got ${provisional.length}`);
  ...
  const finalizedExistingPacketRules = ["Verra.AFOLU.VM0007.v1-8.R-1-0014", "Verra.AFOLU.VM0007.v1-8.R-2-0009", "Verra.AFOLU.VM0007.v1-8.R-2-0010", "Verra.AFOLU.VM0007.v1-8.R-2-0011", "Verra.AFOLU.VM0007.v1-8.R-2-0012", "Verra.AFOLU.VM0007.v1-8.R-2-0015", "Verra.AFOLU.VM0007.v1-8.R-2-0016", "Verra.AFOLU.VM0007.v1-8.R-3-0002"].map((stableRuleId) => ({ stableRuleId, batch: stableRuleId.endsWith("R-1-0014") ? 3 : 4, reviewStatus: "REVIEWED", existingFrozenPacketSufficientForIndependentReview: true, scopeGroup: "CAN_FINALIZE_FROM_EXISTING_PACKET", scopeReason: "Selected by the merged scope manifest for existing-packet independent review; final reviewed truth is recorded separately." }));
  return { schemaVersion: "rc5-maya-provisional-independent-review-scope-v1", purpose: "Deterministic scope inventory for the 19 remaining Maya provisional rules. This manifest contains no new judgments and does not modify reviewed or machine truth.", sourceCommitSha: "ea28e069d2a171df7fc7fff31ef2dc3c6f99ef4b", machineTruth: { path: machineTruthPath, sha256: sha256(fs.readFileSync(path.join(root, machineTruthPath))), proposalState: "MACHINE_PROPOSED" }, inventory: { totalRules: decisions.length, uniqueRuleCount: new Set(decisions.map((decision) => decision.stableRuleId)).size, reviewedRuleCount: decisions.filter((decision) => decision.reviewStatus === "REVIEWED").length, provisionalRuleCount: provisional.length }, groupCounts: Object.fromEntries(["CAN_FINALIZE_FROM_EXISTING_PACKET", "REQUIRES_TARGETED_FULL_PDD_RETRIEVAL", "REQUIRES_METHODOLOGY_EXPERT_INTERPRETATION", "BLOCKED_BY_PROVENANCE_OR_SCHEMA"].map((group) => [group, sorted.filter((rule) => rule.scopeGroup === group).length])), rules: sorted, finalizedExistingPacketRules, recommendedBatches: [
