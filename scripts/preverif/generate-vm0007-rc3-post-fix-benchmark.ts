...
import { evaluateVm0007Benchmark, machineProposalToBenchmarkRows, reviewedTruthToBenchmarkRows } from "../../src/lib/preverif/vm0007Benchmark";
...
const metric = (baseline: number | null, postFix: number | null) => ({ baseline, postFix, delta: postFix === null || baseline === null ? null : postFix - baseline, direction: postFix === null || baseline === null ? "not_comparable" : postFix < baseline ? "improved" : postFix > baseline ? "regressed" : "unchanged" });
const metrics = {
  acceptedEvidenceFalseSupport: metric(baselineEvidence.aggregate.accepted.falsePositiveCount, postFixEvidence.aggregate.accepted.falsePositiveCount),
  acceptedEvidenceMissed: metric(baselineEvidence.aggregate.accepted.falseNegativeCount, postFixEvidence.aggregate.accepted.falseNegativeCount),
  evidenceStateFailures: metric(baselineCategorical.aggregate.fields.evidenceState.mismatchedCount, postFixCategorical.aggregate.fields.evidenceState.mismatchedCount),
  applicabilityFailures: metric(baselineCategorical.aggregate.fields.applicability.mismatchedCount, postFixCategorical.aggregate.fields.applicability.mismatchedCount),
  reviewerOutcomeFailures: metric(baselineCategorical.aggregate.fields.reviewerOutcome.mismatchedCount, postFixCategorical.aggregate.fields.reviewerOutcome.mismatchedCount),
  contradictionFailures: metric(baselineCategorical.aggregate.fields.contradictionState.mismatchedCount, postFixCategorical.aggregate.fields.contradictionState.mismatchedCount),
  draftFindingFailures: metric(baselineCategorical.aggregate.fields.draftFinding.mismatchedCount, postFixCategorical.aggregate.fields.draftFinding.mismatchedCount),
  clientActionFailures: metric(baselineCategorical.aggregate.fields.clientAction.mismatchedCount, postFixCategorical.aggregate.fields.clientAction.mismatchedCount),
  acceptedEvidencePrecision: metric(baselineEvidence.aggregate.accepted.precision, postFixEvidence.aggregate.accepted.precision),
  acceptedEvidenceRecall: metric(baselineEvidence.aggregate.accepted.recall, postFixEvidence.aggregate.accepted.recall),
  acceptedEvidenceF1: metric(baselineEvidence.aggregate.accepted.f1, postFixEvidence.aggregate.accepted.f1),
};
const machineOutput = (row: any) => ({ stableRuleId: row.stableRuleId, upstreamStatus: row.upstreamStatus, proposedApplicability: row.proposedApplicability, reviewerOutcome: row.reviewerOutcome, contradictionState: row.contradictionState, draftFindingCandidate: row.draftFindingCandidate, clientAction: row.clientAction, acceptedEvidence: row.acceptedEvidence, rejectedEvidence: row.rejectedEvidence, proposedAcceptedEvidence: row.proposedAcceptedEvidence, proposedRejectedEvidence: row.proposedRejectedEvidence, quote: row.quote, spanId: row.spanId, provenance: row.provenance });
const changedRuleIds = expectedStableRuleIds.filter((id: string) => canonicalJsonStringify(machineOutput(baselineProposal.rows.find((row: any) => row.stableRuleId === id))) !== canonicalJsonStringify(machineOutput(proposal.rows.find((row: any) => row.stableRuleId === id)))).sort();
...
const changeAudit = changedRuleIds.map((stableRuleId: string) => {
  ...
  const removedFalseSupport = evidenceRemoved.every((item: any) => baselineFalseSupport.has(canonicalJsonStringify(item)));
  const classification = afterBenchmark.accepted.falsePositiveRecords.length < beforeBenchmark.accepted.falsePositiveRecords.length && removedFalseSupport && afterBenchmark.accepted.falseNegativeRecords.length <= beforeBenchmark.accepted.falseNegativeRecords.length ? "intended_improvement" : afterBenchmark.accepted.falsePositiveRecords.length > beforeBenchmark.accepted.falsePositiveRecords.length || afterBenchmark.accepted.falseNegativeRecords.length > beforeBenchmark.accepted.falseNegativeRecords.length ? "regression" : "neutral_representation_change";
  return { stableRuleId, baselineStatus: before.upstreamStatus, postFixStatus: after.upstreamStatus, reviewedStatus: reviewed.finalEvidenceState, baselineAcceptedEvidence: beforeAccepted, postFixAcceptedEvidence: afterAccepted, reviewedAcceptedEvidence: reviewedAccepted, baselineRejectedEvidence: beforeRejected, postFixRejectedEvidence: afterRejected, evidenceAdded, evidenceRemoved, sourceBackedExtraction: { baseline: sourceBacked(beforeAccepted), postFix: sourceBacked(afterAccepted), reviewed: sourceBacked(reviewedAccepted) }, provenanceChanges: { accepted: beforeAccepted.length !== afterAccepted.length || canonicalJsonStringify(beforeAccepted) !== canonicalJsonStringify(afterAccepted) }, findingActionChanges: { draftFinding: before.draftFindingCandidate !== after.draftFindingCandidate, clientAction: before.clientAction !== after.clientAction }, classification, reason: removedFalseSupport && classification === "intended_improvement" ? "Manual review found only baseline false-support evidence removed; reviewed truth and source-backed extraction support the change." : "Manual review against gold.json and the source-backed extraction found no aggregate-safe improvement classification." };
});
...
const benchmark = { schemaVersion: "vm0007-rc3-audited-post-fix-benchmark-v1", baselineIdentity: { path: relative(files.baseline), sha256: expectedHashes.baseline, proposalPath: relative(files.baselineProposal), proposalSha256: expectedHashes.baselineProposal, auditExecutionSha256: expectedHashes.preFixAuditExecution }, reviewedTruthIdentity: { path: relative(files.truth), sha256: expectedHashes.truth }, extractionIdentity: { path: relative(files.extraction), sha256: expectedHashes.extraction }, postFixProposalIdentity: { path: relative(postFixProposalPath), sha256: sha256Text(proposalSerialized) }, postFixAuditExecutionSha256: auditExecutionSha256, ruleCount: 58, stableRuleIds: [...stableRuleIds].sort(), baselineMetrics: metricsFrom(baselineEvidence, baselineCategorical), postFixMetrics: metricsFrom(postFixEvidence, postFixCategorical), metricDeltas: metrics, improvements, regressions, unchangedMetrics, changedRuleIds, gateResult };
...
write(changeAuditPath, `${canonicalJsonStringify({ schemaVersion: "vm0007-rc3-audited-post-fix-change-audit-v1", manualReview: true, changedRuleCount: changeAudit.length, rules: changeAudit })}\n`);
