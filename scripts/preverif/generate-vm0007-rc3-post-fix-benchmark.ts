...
import { changedVm0007RuleIds, validateVm0007ManualReview, VM0007_REVIEW_CLASSIFICATIONS } from "../../src/lib/preverif/vm0007BenchmarkIntegrity";
...
const changedRuleIds = changedVm0007RuleIds(baselineProposal.rows, proposal.rows, expectedStableRuleIds);
...
const benchmark = { schemaVersion: "vm0007-rc3-audited-post-fix-benchmark-v2", baselineIdentity: { path: relative(files.baseline), sha256: expectedHashes.baseline, proposalPath: relative(files.baselineProposal), proposalSha256: expectedHashes.baselineProposal, auditExecutionSha256: expectedHashes.preFixAuditExecution }, reviewedTruthIdentity: { path: relative(files.truth), sha256: expectedHashes.truth }, extractionIdentity: { path: relative(files.extraction), sha256: expectedHashes.extraction }, manualReviewIdentity: { path: relative(files.manualReview), sha256: sha256Bytes(files.manualReview) }, postFixProposalIdentity: { path: relative(postFixProposalPath), sha256: sha256Text(proposalSerialized) }, postFixAuditExecutionSha256: auditExecutionSha256, ruleCount: 58, stableRuleIds: [...stableRuleIds].sort(), baselineMetrics: metricsFrom(baselineEvidence, baselineCategorical), postFixMetrics: metricsFrom(postFixEvidence, postFixCategorical), metricDeltas: metrics, improvements, regressions, unchangedMetrics, changedRuleIds, diagnosticTraceChangedRuleIds: [], gateResult };
...
write(changeAuditPath, `${canonicalJsonStringify({ schemaVersion: "vm0007-rc3-audited-post-fix-change-audit-v2", manualReview: false, manualReviewInput: { path: relative(files.manualReview), sha256: sha256Bytes(files.manualReview) }, changedRuleCount: changeAudit.length, rules: changeAudit })}\n`);
