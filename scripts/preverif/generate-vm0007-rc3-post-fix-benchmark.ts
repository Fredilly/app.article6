...
const legacyRelative = (file: string) => relative(file)
...
const postFixProposalPath = path.join(baselineDir, "RC3_AUDITED_POST_FIX_PROPOSAL.json");
write(postFixProposalPath, proposalSerialized);
const benchmark = { schemaVersion: "vm0007-rc3-audited-post-fix-benchmark-v4", baselineIdentity: { path: legacyRelative(files.baseline), sha256: expectedHashes.baseline, proposalPath: legacyRelative(files.baselineProposal), proposalSha256: expectedHashes.baselineProposal, auditExecutionSha256: expectedHashes.preFixAuditExecution, diagnosticPath: legacyRelative(files.diagnostic), diagnosticSha256: expectedHashes.diagnostic }, reviewedTruthIdentity: { path: legacyRelative(files.truth), sha256: expectedHashes.truth }, extractionIdentity: { path: legacyRelative(files.extraction), sha256: expectedHashes.extraction }, manualReviewIdentity: { path: legacyRelative(files.manualReview), sha256: sha256Bytes(files.manualReview) }, postFixProposalIdentity: { path: legacyRelative(postFixProposalPath), sha256: sha256Text(proposalSerialized) }, postFixAuditExecutionSha256: auditExecutionSha256, ruleCount: 58, stableRuleIds: [...stableRuleIds].sort(), baselineMetrics: metricsFrom(baselineEvidence, baselineCategorical), postFixMetrics: metricsFrom(postFixEvidence, postFixCategorical), metricDeltas: metrics, improvements, regressions, unchangedMetrics, serializedRowChangedRuleIds, acceptedEvidenceMissDiagnosticChangedRuleIds, acceptedEvidenceMissDiagnosticChangedRuleCount: acceptedEvidenceMissDiagnosticChangedRuleIds.length, changedRuleIds, changedRuleCount: changedRuleIds.length, gateResult };
...
write(changeAuditPath, `${canonicalJsonStringify({ schemaVersion: "vm0007-rc3-audited-post-fix-change-audit-v3", manualReview: false, manualReviewInput: { path: legacyRelative(files.manualReview), sha256: sha256Bytes(files.manualReview) }, changedRuleCount: changeAudit.length, rules: changeAudit })}\n`);
...
const report = `# RC3 audited post-fix benchmark\n\n- Frozen baseline: \`${legacyRelative(files.baseline)}\` (${expectedHashes.baseline})\n- Frozen pre-fix proposal: \`${legacyRelative(files.baselineProposal)}\` (${expectedHashes.baselineProposal})\n- Reviewed truth: \`${legacyRelative(files.truth)}\` (${expectedHashes.truth})\n- Raw extraction: \`${legacyRelative(files.extraction)}\` (${expectedHashes.extraction})\n- Frozen pre-fix diagnostic: \`${legacyRelative(files.diagnostic)}\` (${expectedHashes.diagnostic})\n- Human review input: \`${legacyRelative(files.manualReview)}\` (${sha256Bytes(files.manualReview)})\n- Post-fix audit execution: ${auditExecutionSha256}\n\n| Metric | Before | After | Delta |\n| --- | ---: | ---: | ---: |\n${Object.entries(metrics).map(([name, value]) => `| ${name} | ${String(value.baseline)} | ${String(value.postFix)} | ${String(value.delta)} |`).join("\n")}\n\nSerialized-row changes: ${serializedRowChangedRuleIds.length}. Accepted-evidence-miss diagnostic changes: ${acceptedEvidenceMissDiagnosticChangedRuleIds.length}. Changed rules reviewed from authored input: ${changeAudit.length}. Improvements: ${improvements.length ? improvements.join(", ") : "none"}. Regressions: ${regressions.length ? regressions.join(", ") : "none"}. Phase 3 gate: **${gateResult}**.\n\nNo production logic or reviewed/frozen artifact was changed by this benchmark.\n`;
write(reportPath, report);
const artifactPaths = [postFixProposalPath, benchmarkPath, changeAuditPath, reportPath];
const manifest = { schemaVersion: "vm0007-rc3-audited-post-fix-manifest-v4", command: "npm run preverif:rc3:post-fix-benchmark", inputs: Object.fromEntries(Object.entries(files).map(([name, file]) => [name, { path: legacyRelative(file), sha256: name === "baseline" ? expectedHashes.baseline : name === "baselineProposal" ? expectedHashes.baselineProposal : name === "truth" ? expectedHashes.truth : name === "extraction" ? expectedHashes.extraction : name === "diagnostic" ? expectedHashes.diagnostic : sha256Bytes(file) }])), artifacts: Object.fromEntries(artifactPaths.map((file) => [legacyRelative(file), { sha256: sha256Bytes(file) }])), manualReview: { path: legacyRelative(files.manualReview), sha256: sha256Bytes(files.manualReview) }, preFixDiagnostic: { path: legacyRelative(files.diagnostic), sha256: expectedHashes.diagnostic }, postFixAuditExecutionSha256: auditExecutionSha256, ruleCount: 58, serializedRowChangedRuleCount: serializedRowChangedRuleIds.length, acceptedEvidenceMissDiagnosticChangedRuleCount: acceptedEvidenceMissDiagnosticChangedRuleIds.length, changedRuleCount: changeAudit.length, gateResult };
