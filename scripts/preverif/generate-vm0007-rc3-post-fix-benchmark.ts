...
import { changedVm0007RuleIds, mapDiagnosticTracesByRuleId, substantiveDiagnosticTrace, validateVm0007ManualReview, VM0007_REVIEW_CLASSIFICATIONS } from "../../src/lib/preverif/vm0007BenchmarkIntegrity";
...
const diagnosticTraceChangedRuleIds = expectedStableRuleIds.filter((id: string) => canonicalJsonStringify(substantiveDiagnosticTrace(frozenDiagnosticById.get(id))) !== canonicalJsonStringify(substantiveDiagnosticTrace(postFalseNegativeTraceById.get(id)))).sort();
